...
/**
 *  @file   rapp_test_expand_bin.c
 *  @brief  Correctness tests for binary 2x expansion.
 */

#include <string.h>              /* memcpy()       */
#include "rapp.h"                /* RAPP API       */
#include "rapp_ref_expand_bin.h" /* Reference API  */
...
bool
rapp_test_expand_1x2_bin(void)
{
    return rapp_test_driver(&rapp_expand_1x2_bin,
                            &rapp_ref_expand_1x2_bin,
                            2, 1);
}

bool
rapp_test_expand_2x1_bin(void)
{
    return rapp_test_driver(&rapp_expand_2x1_bin,
                            &rapp_ref_expand_2x1_bin,
                            1, 2);
}

bool
rapp_test_expand_2x2_bin(void)
{
    return rapp_test_driver(&rapp_expand_2x2_bin,
                            &rapp_ref_expand_2x2_bin,
                            2, 2);
}
    ...
    for (k = 0; k < RAPP_TEST_ITER; k++) {
        ...
        if (/* src == dst */
            ...
            || (*func)(dst_buf, dst_dim,
                       dst_buf + dst_dim*(yscale*height - 1) +
                       rapp_align((xscale*width + 7) / 8) - rapp_alignment,
