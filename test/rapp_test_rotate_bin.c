...
/**
 *  @file   rapp_test_rotate_bin.c
 *  @brief  Correctness tests for binary image rotation.
 */

#include <string.h>               /* memcpy()      */
#include "rapp.h"                 /* RAPP API      */
#include "rapp_ref_rotate_bin.h"  /* Reference API */
...
bool
rapp_test_rotate_cw_bin(void)
{
    return rapp_test_rotate_driver(&rapp_rotate_cw_bin,
                                   &rapp_ref_rotate_cw_bin);
}

bool
rapp_test_rotate_ccw_bin(void)
{
    return rapp_test_rotate_driver(&rapp_rotate_ccw_bin,
                                   &rapp_ref_rotate_ccw_bin);
}
...
rapp_test_rotate_driver(int (*test)(), void (*ref)())
{
    int      dst_dim = rapp_align((RAPP_TEST_HEIGHT + 7) / 8);
    ...
    for (k = 0; k < RAPP_TEST_ITER; k++) {
        ...
        if (/* src == dst */
            ...
            || (*test)(dst_buf, dst_dim,
                       ...
                       rapp_align((height + 7) / 8) - rapp_alignment,
                                   ...
                                   dst_dim, 0, height, width))
        {
            ...
            DBG("dst=\n");
            rapp_test_dump_bin(dst_buf, dst_dim, 0, height, width);
            DBG("ref=\n");
            rapp_test_dump_bin(ref_buf, dst_dim, 0, height, width);
            goto Done;
        }
    }
