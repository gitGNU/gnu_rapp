...
/**
 *  @file   rapp_test_scatter.c
 *  @brief  Correctness tests for 8-bit scatter.
 */

#include <string.h>           /* memcpy()      */
#include "rapp.h"             /* RAPP API      */
#include "rapp_ref_scatter.h" /* Reference API */
...
bool
rapp_test_scatter_u8(void)
{
    return rapp_test_scatter_driver(&rapp_scatter_u8,
                                    &rapp_ref_scatter_u8);
}
...
rapp_test_scatter_driver(int (*test)(), int (*ref)())
{
    ...
    uint8_t *pack_buf = rapp_malloc(RAPP_TEST_WIDTH*RAPP_TEST_HEIGHT, 0);
    bool     pass     = false;
    int      k;

    /* Initialize the packed pixel buffer */
    rapp_test_init(pack_buf, 0, RAPP_TEST_WIDTH*RAPP_TEST_HEIGHT, 1, false);
    ...
    for (k = 0; k < RAPP_TEST_ITER; k++) {
                                  ...
                                  width, height))
        {
            ...
            DBG("pack=\n");
            rapp_test_dump_u8(pack_buf, 0, rlen, 1);
            DBG("dst=\n");
