...
do {                                                                     \
    ...
    (dstv) = vec_vperm(vec2, vec__, vec1);              /* Permute    */ \
...
do {                                           \
    ...
    adj__  = vec_and(adj__, (rc_vec_t)(0x01)); \
...
do {                                           \
    ...
    rc_vec_t cmp__ = vec_cmpgt(sv2__, sv1__);  \
    xor__  = vec_and(xor__, cmp__);            \
    xor__  = vec_and(xor__, (rc_vec_t)(0x01)); \
...
do {                                          \
    rc_vec_t vec__;                           \
    vec__  = vec_xor(srcv, (rc_vec_t)(0x80)); \
    vec__  = vec_abs((vector int8_t)vec__);   \
...
do {                                         \
    rc_vec_t gt__ = vec_cmpgt(srcv2, srcv1); \
...
#define RC_VEC_BLEND(blendv, blend8) \
    RC_VEC_SPLAT__(blendv, (blend8) << 7, uint16_t)
...
do {                                                         \
    rc_vec_t       sv1__ = (srcv1);                          \
    rc_vec_t       sv2__ = (srcv2);                          \
    vector int16_t bv__  = (blendv);                         \
    vector int16_t lo1__ = vec_mergel((rc_vec_t)(0), sv1__); \
    vector int16_t hi1__ = vec_mergeh((rc_vec_t)(0), sv1__); \
    vector int16_t lo2__ = vec_mergel((rc_vec_t)(0), sv2__); \
    vector int16_t hi2__ = vec_mergeh((rc_vec_t)(0), sv2__); \
    lo2__  = vec_sub(lo2__, lo1__);                          \
    hi2__  = vec_sub(hi2__, hi1__);                          \
    lo1__  = vec_mradds(lo2__, bv__, lo1__);                 \
    hi1__  = vec_mradds(hi2__, bv__, hi1__);                 \
    (dstv) = vec_pack(hi1__, lo1__);                         \
} while (0)
...
#define RC_VEC_BLENDZ(blendv, blend8) \
    RC_VEC_SPLAT__(blendv, blend8, uint16_t)
...
do {                                                      \
    rc_vec_t sv1__ = (srcv1);                             \
    rc_vec_t sv2__ = (srcv2);                             \
    rc_vec_t bias__ = vec_cmpgt(sv1__, sv2__);            \
...
do {                                                      \
    rc_vec_t sv1__ = (srcv1);                             \
    rc_vec_t sv2__ = (srcv2);                             \
    rc_vec_t bias__ = vec_cmpgt(sv2__, sv1__);            \
    RC_VEC_LERP__(dstv, sv1__, sv2__, blend8v, bias__);   \
} while (0)
...
do {                                                                    \
    rc_vec_t        vec__  = (vec);                                     \
    vector unsigned mask__ = (vector unsigned)(0x80402010, 0x08040201,  \
                                               0x80402010, 0x08040201); \
    vec__   = vec_cmpgt(vec__, (rc_vec_t)(0x7f));                       \
    mask__  = vec_and(vec__, (rc_vec_t)mask__);                         \
    mask__  = vec_sum4s((rc_vec_t)mask__, (vector unsigned)(0));        \
    mask__  = vec_sl(mask__, (vector unsigned)(8, 8, 0, 0));            \
    mask__  = vec_sums((vector int)mask__, (vector int)(0));            \
    (maskv) = vec_slo((rc_vec_t)mask__, (rc_vec_t)(8*14));              \
} while (0)
...
do {                                                                     \
    vector int c1__  = (vector int)(0x55555555);                         \
    vector int c2__  = (vector int)(0x33333333);                         \
    vector int c4__  = (vector int)(0x0f0f0f0f);                         \
    vector int acc__ = (srcv);                                           \
    vector int tmp__;                                                    \
    tmp__  = vec_sr(acc__, (vector unsigned)(1)); /* tmp  = acc >> 1  */ \
    tmp__  = vec_and(tmp__, c1__);                /* tmp &= 0x55...   */ \
    acc__  = vec_and(acc__, c1__);                /* acc &= 0x55...   */ \
    acc__  = vec_add(acc__, tmp__);               /* acc += tmp       */ \
    tmp__  = vec_sr(acc__, (vector unsigned)(2)); /* tmp  = acc >> 2  */ \
    tmp__  = vec_and(tmp__, c2__);                /* tmp &= 0x33...   */ \
    acc__  = vec_and(acc__, c2__);                /* acc &= 0x33...   */ \
    acc__  = vec_add(acc__, tmp__);               /* acc += tmp       */ \
    tmp__  = vec_sr(acc__, (vector unsigned)(4)); /* tmp  = acc >> 4  */ \
    acc__  = vec_add(acc__, tmp__);               /* acc += tmp       */ \
...
#define RC_VEC_SUMV(accv, srcv) \
    ((accv) = vec_sum4s(srcv, (vector unsigned)(accv)))

/**
 *  Sum all 8-bit fields, reduction step.
 */
#define RC_VEC_SUMR(sum, accv) \
    RC_VEC_VTOI__(sum, vec_sums((vector int)(accv), (vector int)(0)), int)
...
#define RC_VEC_MACV(accv, srcv1, srcv2) \
    ((accv) = vec_msum(srcv1, srcv2, (vector unsigned)(accv)))
...
do {                                           \
    ...
    map__ = vec_splat((vector type)map__, 0);  \
    (vec) = vec_perm(vec__, vec__, map__);     \
} while (0)
...
do {                                                         \
    vector int16_t bv__  = (blendv);                         \
    vector int16_t lob__ = vec_mergel((rc_vec_t)(0), bias);  \
    vector int16_t hib__ = vec_mergeh((rc_vec_t)(0), bias);  \
    vector int16_t lo1__ = vec_mergel((rc_vec_t)(0), srcv1); \
    vector int16_t hi1__ = vec_mergeh((rc_vec_t)(0), srcv1); \
    vector int16_t lo2__ = vec_mergel((rc_vec_t)(0), srcv2); \
    vector int16_t hi2__ = vec_mergeh((rc_vec_t)(0), srcv2); \
    lo2__  = vec_sub(lo2__, lo1__);                          \
    hi2__  = vec_sub(hi2__, hi1__);                          \
    lo1__  = vec_mladd(lo2__, bv__, lob__);                  \
    hi1__  = vec_mladd(hi2__, bv__, hib__);                  \
    lo1__  = vec_sr(lo1__, (vector uint16_t)(8));            \
    hi1__  = vec_sr(hi1__, (vector uint16_t)(8));            \
    (dstv) = vec_pack(hi1__, lo1__);                         \
